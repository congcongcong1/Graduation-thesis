...
\noindent 其中，\texttt{-rc constqp}表示恒定QP码率控制模式。训练阶段遍历5个QP点，即$\mathcal{Q}=\{37,40,42,45,47\}$，并对奖励取平均。\texttt{-preset p4}对应NVENC的Medium编码预设，\texttt{-r 120}表示训练序列的原始帧率。
...
本课题相关代码包括模型训练代码（\path{rapnet_hevc_ppo_qp_yuv_1080p_v2.py}）、推理与评估脚本（\path{eval_rapnet_comprehensive.py}）、FFmpeg编码调用脚本及实验结果统计脚本。完整工程已随毕业设计材料一并提交，论文正文与附录主要用于说明关键实现思路与核心流程。
