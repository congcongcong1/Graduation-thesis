...
# ================================================================
# Quality Metrics - 可选依赖
# ================================================================
try:
    from IQA_pytorch import DISTS as DISTS_Model
    DISTS_AVAILABLE = True
except ImportError:
    DISTS_AVAILABLE = False
    print("[Warn] IQA_pytorch not installed. DISTS metric unavailable.")
...
try:
    ...
    print("[Warn] scipy not installed. Wasserstein metric unavailable.")
...
try:
    ...
    print("[Warn] vmaf package not installed. VMAF metric unavailable.")
    print("      Install with: pip install vmaf")
# ================================================================
# 基础设置
# ================================================================
...
DEVICE_ACTOR = torch.device("cuda:0") if torch.cuda.is_available() else device
DEVICE_CRITIC = torch.device("cuda:3") if torch.cuda.device_count() > 1 else DEVICE_ACTOR
CONFIG = {
    ...
    "device_actor": "cuda:0",   
    "device_critic": "cuda:3",
    
    # ---------- 迭代 & 数据 ----------
    "num_iterations": 20000,    # 论文里就是 5000 iterations
    "clip_len": 16,            # 1080p 一次最多处理 16 帧（论文中对 1080p 的限制）
    "width": 1920,
    "height": 1080,
    # 修改：使用 Random Crop 替代下采样
    "crop_width": 960,   # 1920 / 2，保留高频信息
    "crop_height": 540,  # 1080 / 2
  ...
  # 核心修改：将奖励权重提高
    ...
    # ---------- Quality Metric ----------
    "quality_metric": "lpips",  # Options: "lpips", "vmaf", "dists", "wasserstein"

    # ---------- Actor Loss Weights (消融实验可调) ----------
    "w_mse": 5,    # MSE regularization weight
    "w_hf": 10,    # High-frequency suppression weight
    ...
    "log_dir": os.path.join(DEFAULT_RUN_DIR, "rapnet_hevc_1_30_v44"),
    "model_dir": os.path.join(DEFAULT_CKPT_DIR, "rapnet_hevc_1_30_v44"),
...
def random_crop_yuv420_clip(y, cb, cr, crop_h, crop_w):
    """从YUV420 clip中随机裁剪固定尺寸区域（替代下采样，保留高频信息）
    
    输入:
        y:  (T,1,H,W)
        cb: (T,1,H/2,W/2)
        cr: (T,1,H/2,W/2)
        crop_h: 裁剪高度（必须是2的倍数，用于YUV420色度对齐）
        crop_w: 裁剪宽度（必须是2的倍数）
    
    输出:
        y_crop:  (T,1,crop_h,crop_w)
        cb_crop: (T,1,crop_h/2,crop_w/2)
        cr_crop: (T,1,crop_h/2,crop_w/2)
    """
    T, _, H, W = y.shape
    Hc, Wc = cb.shape[2], cb.shape[3]
    
    assert crop_h % 2 == 0 and crop_w % 2 == 0, "裁剪尺寸必须是2的倍数（YUV420色度对齐要求）"
    assert crop_h <= H and crop_w <= W, f"裁剪尺寸({crop_h}x{crop_w})不能超过原始尺寸({H}x{W})"
    
    # 随机选择裁剪起始位置（确保色度对齐：坐标必须是2的倍数）
    ...
    # 裁剪Cb/Cr通道（色度位置对应亮度的一半）
    top_c = top // 2
    left_c = left // 2
    crop_h_c = crop_h // 2
    crop_w_c = crop_w // 2
    
    cb_crop = cb[:, :, top_c:top_c+crop_h_c, left_c:left_c+crop_w_c]
    cr_crop = cr[:, :, top_c:top_c+crop_h_c, left_c:left_c+crop_w_c]
...
# Global DISTS model instance
_dists_model = None

def init_dists_model():
    """Initialize DISTS model (lazy loading)"""
    global _dists_model
    if _dists_model is None and DISTS_AVAILABLE:
        print(f"[*] Initializing DISTS on {LPIPS_DEVICE}...")
        _dists_model = DISTS_Model().to(LPIPS_DEVICE)
        _dists_model.eval()
    return _dists_model


def compute_vmaf_score(ref_clip_bgr, dec_clip_bgr):
    """使用SSIM作为VMAF的临时替代(VMAF需要特殊配置)"""
    try:
        from skimage.metrics import structural_similarity as ssim
    except ImportError:
        raise RuntimeError("scikit-image required for VMAF approximation")
    
    T = ref_clip_bgr.shape[0]
    scores = []
    for t in range(T):
        score = ssim(ref_clip_bgr[t], dec_clip_bgr[t], channel_axis=2, data_range=255)
        scores.append(score)
    return float(np.mean(scores))


def compute_dists_score(ref_clip_bgr, dec_clip_bgr):
    """使用DISTS计算感知质量"""
    if not DISTS_AVAILABLE:
        raise RuntimeError("DISTS not available. Install: pip install IQA-PyTorch")
    
    model = init_dists_model()
    if model is None:
        raise RuntimeError("DISTS model initialization failed")
    ...
    with torch.no_grad():
        for t in range(T):
            ref_frame = ref_clip_bgr[t][..., ::-1].copy()
            dec_frame = dec_clip_bgr[t][..., ::-1].copy()
            
            ref_tensor = torch.from_numpy(ref_frame).permute(2, 0, 1).float().unsqueeze(0) / 255.0
            dec_tensor = torch.from_numpy(dec_frame).permute(2, 0, 1).float().unsqueeze(0) / 255.0
            
            ref_tensor = ref_tensor.to(LPIPS_DEVICE)
            dec_tensor = dec_tensor.to(LPIPS_DEVICE)
            
            dist = model(ref_tensor, dec_tensor)
            scores.append(dist.item())
            
            del ref_tensor, dec_tensor, dist
    
    avg_dist = np.mean(scores)
    quality = -float(avg_dist)
    return quality


def compute_wasserstein_score(ref_clip_bgr, dec_clip_bgr):
    """使用Wasserstein距离计算分布差异（优化版：采样加速）"""
    if not SCIPY_AVAILABLE:
        raise RuntimeError("scipy required for Wasserstein distance")
    
    T = ref_clip_bgr.shape[0]
    distances = []
    
    # 采样数量（10000足够估计分布，比518400快50倍）
    sample_size = 10000
    
    for t in range(T):
        ref_frame = ref_clip_bgr[t].astype(np.float32) / 255.0
        dec_frame = dec_clip_bgr[t].astype(np.float32) / 255.0
        
        channel_dists = []
        for c in range(3):
            ref_channel = ref_frame[:, :, c].flatten()
            dec_channel = dec_frame[:, :, c].flatten()
            
            # 随机采样加速（如果像素数大于采样数）
            if len(ref_channel) > sample_size:
                indices = np.random.choice(len(ref_channel), sample_size, replace=False)
                ref_channel = ref_channel[indices]
                dec_channel = dec_channel[indices]
            
            dist = wasserstein_distance(ref_channel, dec_channel)
            channel_dists.append(dist)
        
        frame_dist = np.mean(channel_dists)
...
def compute_quality_metric_unified(ref_clip_bgr, dec_clip_bgr, metric="lpips"):
    """统一质量评估接口"""
    if metric == "lpips":
        return compute_quality_metric(ref_clip_bgr, dec_clip_bgr)
    elif metric == "vmaf":
        return compute_vmaf_score(ref_clip_bgr, dec_clip_bgr)
    elif metric == "dists":
        return compute_dists_score(ref_clip_bgr, dec_clip_bgr)
    elif metric == "wasserstein":
        return compute_wasserstein_score(ref_clip_bgr, dec_clip_bgr)
    else:
...
class DeterministicACAgentYCbCr:
    ...
    def update(self,
        ...
        w_low = 0.5
        w_hf  = self.w_hf   # 从构造函数获取，消融实验可配置
        w_edge= 0.2
        w_tv  = 10
        w_id  = 0.01
        ...
        w_mse = self.w_mse  # 从构造函数获取，消融实验可配置
...
class NvencClipEnvQP:
    """
    视频编码强化学习环境 - 基于HEVC_NVENC（使用Random Crop策略）
    ...
    环境工作流程:
      1. reset(): 从视频库随机采样一个clip (16帧)
         - 读取1080p YUV420原始帧
         - 随机裁剪到编码分辨率(540x960)作为状态S（保留高频信息）

      2. step(action): 接收预处理后的帧(动作A)
         - 对每个QP(37,40,42,45,47)进行HEVC编码
         - 解码（无需上采样！直接在裁剪分辨率上计算）
         - 计算RD奖励: R = α*ΔV + β*ΔR
           * ΔV: 质量提升(LPIPS)
           * ΔR: 码率节省

    状态空间(State):
      - 随机裁剪后的YUV420帧: Y(T,1,crop_H,crop_W), Cb/Cr(T,1,crop_H/2,crop_W/2)
      - 默认: Y(16,1,540,960), Cb/Cr(16,1,270,480)
    ...
    def __init__(self,
                 ...
                 width: int = 1920,
                 height: int = 1080,
                 crop_width: int = 960,   # 修改：替代 downscale
                 crop_height: int = 540,  # 修改：替代 downscale
                 ffmpeg_bin: str = "ffmpeg",
                 alpha_q: float = 5,
                 beta_r: float = 5,
                 quality_metric: str = "lpips"):
        ...
        self.W = width
        self.H = height
        self.crop_W = crop_width    # 修改：使用裁剪尺寸
        self.crop_H = crop_height   # 修改：使用裁剪尺寸
    ...
    def reset(self):
        """
        重置环境,开始新的episode（使用Random Crop替代下采样）

        工作流程:
          1. 从视频库随机选择一个视频
          2. 从该视频随机采样一个clip(16帧)
          3. 读取1080p YUV420原始帧
          4. 随机裁剪到编码分辨率作为状态（保留高频信息）
          5. 保存裁剪后的BGR帧用于后续LPIPS计算

        Returns:
            y_crop, cb_crop, cr_crop: 裁剪后的YUV420状态
              - y_crop:  (T,1,crop_H,crop_W)
              - cb_crop: (T,1,crop_H/2,crop_W/2)
              - cr_crop: (T,1,crop_H/2,crop_W/2)
            info: {'video_id': str}
        ...
        # 2) 随机裁剪到编码分辨率（保留高频信息，替代下采样）
        y_crop, cb_crop, cr_crop = random_crop_yuv420_clip(
            y_1080, cb_1080, cr_1080, self.crop_H, self.crop_W
        )

        # 3) 保存裁剪后的参考 BGR（用于 LPIPS 计算，无需上采样！）
        self.ref_clip_bgr = yuv420_planes_to_bgr_clip(y_crop, cb_crop, cr_crop)

        # 保存状态供step()使用
        self.state_y = y_crop
        self.state_cb = cb_crop
        self.state_cr = cr_crop

        # 确保数值范围在[0,1]
        y_crop = np.clip(y_crop, 0.0, 1.0)
        cb_crop = np.clip(cb_crop, 0.0, 1.0)
        cr_crop = np.clip(cr_crop, 0.0, 1.0)
        ...
        if DEBUG_SHAPES:
            ...
            print("  y_crop:", y_crop.shape, "cb_crop:", cb_crop.shape, "cr_crop:", cr_crop.shape)
            print("  y_crop range:", float(y_crop.min()), float(y_crop.max()))
        return y_crop, cb_crop, cr_crop, info
    ...
    def step(self,
             ...
             cr_action: np.ndarray):
        """
        执行动作(预处理后的帧),计算RD奖励（使用Random Crop策略）

        工作流程:
          1. 将预处理后的YUV420帧写入临时文件
          2. 对每个QP点(37,40,42,45,47)进行HEVC编码
             a) 使用HEVC_NVENC编码（在裁剪分辨率上）
             b) 解码（无需上采样！直接在裁剪分辨率上计算）
             c) 计算质量(LPIPS)和码率
             d) 与baseline对比,计算RD增益
          3. 对所有QP点的RD增益求平均作为奖励

        Args:
            y_action:  (T,1,crop_H,crop_W) 预处理后的亮度分量
            cb_action: (T,1,crop_H/2,crop_W/2) 预处理后的Cb分量
            cr_action: (T,1,crop_H/2,crop_W/2) 预处理后的Cr分量
        ...
        """
        T = self.clip_len
        H_enc = self.crop_H  # 修改：使用裁剪尺寸
        W_enc = self.crop_W  # 修改：使用裁剪尺寸
        Hc = H_enc // 2
        Wc = W_enc // 2

        # 验证动作维度
        assert y_action.shape == (T, 1, H_enc, W_enc), \
            f"Expected y_action shape ({T},1,{H_enc},{W_enc}), got {y_action.shape}"
        ...
        with tempfile.TemporaryDirectory(dir=self.work_dir) as tmpdir:
            ...
            for qp in self.qp_list:
                ...
                # 计算质量分数(使用配置的指标,取负值使越大越好)
                V_cur = compute_quality_metric_unified(self.ref_clip_bgr, dec_clip_bgr, self.quality_metric)
...
def build_baseline_rd_json_qp_clips(
    ...
    for meta in video_meta_list:
        ...
        for i in range(samples_per_video):
            ...
            # 2) 随机裁剪到编码分辨率（保留高频信息，替代下采样）
            y_crop, cb_crop, cr_crop = random_crop_yuv420_clip(
                y_1080, cb_1080, cr_1080, crop_height, crop_width
            )

            # 3) 构造裁剪后的 BGR 参考 (用于 LPIPS，无需上采样！)
            ref_clip_bgr = yuv420_planes_to_bgr_clip(y_crop, cb_crop, cr_crop)
            
            # 准备输入文件 (Clip 级别，使用裁剪后的数据)
            yuv_bytes_input = ycbcr420_clip_to_yuv420_bytes(y_crop, cb_crop, cr_crop)
            ...
            for qp in qp_list:
                ...
                # 读取解码结果（无需上采样！直接使用裁剪分辨率）
                y_dec, cb_dec, cr_dec = read_yuv420_clip_to_planes(
                    dec_yuv_path, clip_len, H_enc, W_enc
                )
                # 直接转BGR，无需上采样
                dec_clip_bgr = yuv420_planes_to_bgr_clip(y_dec, cb_dec, cr_dec)

                V_val = compute_quality_metric_unified(ref_clip_bgr, dec_clip_bgr, quality_metric)
...
def  train_rapnet_with_config(
        ...
        width: int = None,
        height: int = None,
        crop_width: int = None,   # 修改：替代 downscale
        crop_height: int = None,  # 修改：替代 downscale
        ffmpeg_bin: str = None):
    """
    ...
    if crop_width is None:
        crop_width = CONFIG["crop_width"]  # 修改
    if crop_height is None:
        crop_height = CONFIG["crop_height"]  # 修改
    if ffmpeg_bin is None:
        ffmpeg_bin = CONFIG["ffmpeg_bin"]

    device_actor, device_critic = resolve_devices()
    H_enc = crop_height  # 修改：使用裁剪尺寸
    W_enc = crop_width   # 修改：使用裁剪尺寸
    ...
    env = NvencClipEnvQP(
        ...
        width=width,
        height=height,
        crop_width=crop_width,    # 修改
        crop_height=crop_height,  # 修改
        ffmpeg_bin=ffmpeg_bin,
        alpha_q=CONFIG["alpha_q"],
        beta_r=CONFIG["beta_r"],
        quality_metric=CONFIG["quality_metric"],  # 新增：传递质量指标
    )
    ...
    agent = DeterministicACAgentYCbCr(
        ...
        w_mse=CONFIG.get("w_mse", 5.0),
        w_hf=CONFIG.get("w_hf", 10.0),
    )
...
def evaluate_agent_once(env: NvencClipEnvQP,
    ...
    for meta in eval_metas:
        # 1. 重置环境到特定视频 (这会填充 env.ref_clip_bgr 和相关状态)
        env.current_video_id = meta["id"]
        y_1080, cb_1080, cr_1080 = load_clip_yuv_planes(meta, env.clip_len)
        # 使用随机裁剪（与训练一致）
        y_crop, cb_crop, cr_crop = random_crop_yuv420_clip(
            y_1080, cb_1080, cr_1080, env.crop_H, env.crop_W
        )
        env.ref_clip_bgr = yuv420_planes_to_bgr_clip(y_crop, cb_crop, cr_crop)
        env.state_y, env.state_cb, env.state_cr = y_crop, cb_crop, cr_crop

        # 2. Actor 执行动作（使用裁剪后的数据）
        y_a, cb_a, cr_a = agent.act(y_crop, cb_crop, cr_crop)
...
def debug_once():
    """
    Debug函数：测试一次完整的数据流，打印所有维度信息
    用于验证Random Crop实现是否正确
    """
    global DEBUG_SHAPES
    DEBUG_SHAPES = True  # 强制开启DEBUG模式
    
    print("\n" + "="*70)
    print("DEBUG_ONCE: 测试Random Crop数据流")
    print("="*70 + "\n")
    ...
    # 创建一个简单的测试视频meta（使用第一个视频）
    test_meta = VIDEO_META_LIST[0]
    print(f"测试视频: {test_meta['id']}")
    print(f"  原始尺寸: {test_meta['width']}x{test_meta['height']}")
    print()
    ...
    env = NvencClipEnvQP(
        video_meta_list=[test_meta],
        rd_curve_json=os.environ.get(
    ...
    print(f"  环境裁剪尺寸: {env.crop_W}x{env.crop_H}")
    print()
    ...
    print(f"  Agent输入尺寸: {agent.H_enc}x{agent.W_enc}")
    print()
    
    # 测试reset
    print("3. 测试 env.reset()...")
    y_s, cb_s, cr_s, info = env.reset()
    print(f"  返回状态维度:")
    print(f"    y_s:  {y_s.shape}  (期望: ({clip_len}, 1, {crop_height}, {crop_width}))")
    print(f"    cb_s: {cb_s.shape}  (期望: ({clip_len}, 1, {crop_height//2}, {crop_width//2}))")
    print(f"    cr_s: {cr_s.shape}  (期望: ({clip_len}, 1, {crop_height//2}, {crop_width//2}))")
    print(f"  video_id: {info['video_id']}")
    print()
    
    # 测试act
    print("4. 测试 agent.act()...")
    agent.actor.eval()  # 评估模式
    y_a, cb_a, cr_a = agent.act(y_s, cb_s, cr_s)
    print(f"  返回动作维度:")
    print(f"    y_a:  {y_a.shape}  (期望: ({clip_len}, 1, {crop_height}, {crop_width}))")
    print(f"    cb_a: {cb_a.shape}  (期望: ({clip_len}, 1, {crop_height//2}, {crop_width//2}))")
    print(f"    cr_a: {cr_a.shape}  (期望: ({clip_len}, 1, {crop_height//2}, {crop_width//2}))")
    print()
    
    # 测试step
    print("5. 测试 env.step()...")
    print("  (这将进行实际的编码/解码，可能需要几秒钟...)")
    _, reward, done, info_step = env.step(y_a, cb_a, cr_a)
    print(f"  Reward: {reward:.4f}")
    print(f"  Done: {done}")
    if "qp_list" in info_step:
        print(f"  QP列表: {info_step['qp_list']}")
        if "R_cur" in info_step:
            print(f"  当前码率: {[f'{r:.2f}' for r in info_step['R_cur']]}")
        if "V_cur" in info_step:
            print(f"  当前质量: {[f'{v:.4f}' for v in info_step['V_cur']]}")
    print()
    
    # 测试update
    print("6. 测试 agent.update()...")
    agent.actor.train()  # 训练模式
    loss_actor, loss_critic = agent.update(
        y_state=y_s,
        cb_state=cb_s,
        cr_state=cr_s,
    ...
    print(f"  Actor Loss: {loss_actor:.6f}")
    print(f"  Critic Loss: {loss_critic:.6f}")
    print()
    
    print("="*70)
    print("✅ DEBUG_ONCE 完成！所有维度检查通过！")
    print("="*70)
    
    DEBUG_SHAPES = False  # 恢复DEBUG模式
    return True
...
VIDEO_META_LIST = [
    {
        "id": "beauty_1080p",
        "path": "/datasets/UVG/Beauty_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
        "fps": 120,
    },
    {
        "id": "bosphorus_1080p",
        "path": "/datasets/UVG/Bosphorus_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
        "fps": 120,
    },
    {
        "id": "honeybee_1080p",
        "path": "/datasets/UVG/HoneyBee_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
        "fps": 120,
    },
    {
        "id": "jockey_1080p",
        "path": "/datasets/UVG/Jockey_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
        "fps": 120,
    },
    {
        ...
        "path": "/datasets/UVG/ReadySteadyGo_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
    },
    {
        "id": "shakendry_1080p",
        "path": "/datasets/UVG/ShakeNDry_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
        "fps": 120,
    },
    ...
    {
        "id": "yachtride_1080p",
        "path": "/datasets/UVG/YachtRide_1920x1080_120fps_420_8bit_YUV.yuv",
        "width": 1920,
        "height": 1080,
    }
]
...
if __name__ == "__main__":
    ...
    # ========== 选项1: 运行Debug测试 ==========
    # 取消注释下面这行来测试Random Crop数据流
    # debug_once()
    
    # ========== 选项2: 构建Baseline（必须先做！） ==========
    # 取消注释下面的代码来重建baseline（使用Random Crop）
    # for metric in["dists","wasserstein","vmaf"]:
    #     build_baseline_rd_json_qp_clips(
    #         video_meta_list,
    #         output_json_path=f"/workspace/RL/ElegantRL-master/my_RL/result/rd_baseline_crop30_{metric}.json",
    #         samples_per_video=30,  # 先用30测试，确认无误后改为50+
    #         clip_len=CONFIG["clip_len"],
    #         qp_list=CONFIG["qp_list"],
    #         fps=CONFIG["fps"],
    #         crop_height=CONFIG["crop_height"],  # 修改：使用crop参数
    #         crop_width=CONFIG["crop_width"],    # 修改：使用crop参数
    #         ffmpeg_bin=CONFIG["ffmpeg_bin"],
    #         quality_metric=metric
    #     )
    
    # ========== 选项3: 开始训练多指标==========
    # 注意：必须先运行选项2构建新的baseline！
    # train_rapnet_with_config(
    #     video_meta_list=video_meta_list,
    #     # rd_json_path="/workspace/RL/ElegantRL-master/my_RL/result/rd_baseline_hevc_qp_full_clip_lowbits_alex.json",
    #     rd_json_path="/workspace/RL/ElegantRL-master/my_RL/result/rd_baseline_crop.json",  # 使用新baseline
    # )
    
    for metric in ["vmaf", "dists", "wasserstein"]:  # Skip lpips if already tested
        CONFIG["quality_metric"] = metric
        CONFIG["num_iterations"] = 5000
        CONFIG["log_dir"] = os.path.join(DEFAULT_RUN_DIR, f"rapnet_hevc_2_11_v47_test_{metric}")
        CONFIG["model_dir"] = os.path.join(DEFAULT_CKPT_DIR, f"rapnet_hevc_2_11_v47_test_{metric}")
        train_rapnet_with_config(
        video_meta_list=video_meta_list,
        rd_json_path=os.path.join(DEFAULT_RESULT_DIR, f"rd_baseline_crop30_{metric}.json"),  # 使用新baseline
        )
